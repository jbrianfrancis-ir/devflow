...
Boot → BlitzOS warm start (`sessions/INDEX.md` + recent records) → `cd` into the member repo → `/flow-status` → drive with `/flow-next` (one step per turn), or `/flow-handsoff` + `/flow-next`.
