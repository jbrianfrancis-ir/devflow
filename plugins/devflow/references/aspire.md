...
## Version policy
Aspire updates **within the current major** apply automatically — bump the package references (`Aspire.*` in AppHost/ServiceDefaults, or `Directory.Packages.props` under central package management) to the latest within-major, e.g. 13.5.0 → 13.5.1 or 13.4 → 13.5. `aspire update` (and `aspire update --self` for the CLI) is the supported way to move a repo's packages; edit the refs by hand where it isn't available. Verify with restore + `aspire publish` (and `aspire --version`). Update the Aspire version in `.planning/ARCHITECTURE.md` to match and note the deviation. A **major** bump (e.g. 13 → 14) is never automatic: raise a `checkpoint:decision` with the breaking-changes/changelog link and wait for approval.

**A minor is not automatically a safe bump.** Aspire ships breaking changes in minor releases (13.5 renamed `ServiceProvider` to `Services` on hosting context types, obsoleted `PublishAsConnectionString`, and removed `aspire ps --resources`). Auto-apply still stands — but read the release notes' *Breaking changes* section, and treat a restore or `aspire publish` failure after the bump as a `CHECKPOINT`, never as something to route around by pinning back to a neighbouring version.
