...
- **`.claude/settings.json`** — the plugin self-bootstrap block (see conventions.md), so every future session — cloud included — installs DevFlow at start. Merged, never overwritten. Migration adds one key `/flow-new` does not write: **`skillOverrides`**, mapping every installed GSD skill to `"off"`. Both systems own `.planning/`, so one stray `/gsd-*` after migration writes GSD state over files DevFlow now owns; `"off"` drops the skill from the model's listing *and* refuses invocation on both paths (the typed `/name` and the `Skill` tool), which turns the preview's warning from advice into a guarantee. Enumerate the names from the host — your own skill listing is authoritative, else glob the host skill directories for `gsd-*` — and write only what you actually found: never a hardcoded roster (GSD ships new skills faster than this file can track them), and no key at all when the host has none installed. Claude Code enforces `skillOverrides`; Codex has no equivalent and ignores it, so state which guard the project actually got. Clearing the unused descriptions also stops them crowding the skill-listing budget (`skillListingBudgetFraction`, 1% of context by default), which truncates the `flow-*` descriptions the model needs to pick correctly. GSD's user-scope agents (`~/.claude/agents/gsd-*`) have no equivalent setting and stay spawnable — out of a project's reach, worth one line in the migration report rather than a silent gap.
