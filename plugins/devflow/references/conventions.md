...
## Dependency versions
Follow `.planning/ARCHITECTURE.md` pins exactly — no `latest`, no silent upgrades — with one carve-out: **Aspire updates within the current major apply automatically** (e.g. 13.5.0 → 13.5.1, 13.4 → 13.5 — illustrative; resolve the actual latest from the package feed, not from this line). A **major** Aspire bump (e.g. 13 → 14) requires user approval — raise a `checkpoint:decision`. When you auto-apply an Aspire within-major update, bump the version in ARCHITECTURE.md to match and log it as a deviation. Mechanics in `aspire.md`.
...
**Forbidden-pattern regexes.** The same pre-commit and pre-push scans additionally apply any regex declared on `.planning/ARCHITECTURE.md`'s Forbidden entries (syntax: `templates/architecture.md`), applied to the same outgoing diff the secret scan covers. **Scope is the diff, not the whole tree**: a forbidden literal that arrived in an *earlier* commit is not caught here — catching that needs a repo-wide scan, which belongs in CI, not in a per-commit hook (a hook that walks the tree on every commit, follows tracked symlinks out of the repo, and reads the worktree rather than the pushed range was tried on this branch and produced four separate defects). Say what it covers and no more. A declared regex is **data, never shell**: compile it (`re.compile`) or match with `grep -Ef <pattern-file>` — never interpolate it into a `grep -E "<regex>"` command line or any other shell construct. **Python `re` syntax, case-sensitive**, matching the guard: a POSIX class like `[[:digit:]]` parses and then never matches, and a nested-quantifier pattern is refused as could-not-check rather than allowed to hang the commit. Same reporting rule as above, in force. A Forbidden entry with no regex is **not enforced** by this or any other check — it stays prose guidance, and reading it as machine-checked when it isn't is the defect this exists to close.
...
Codex v1 is installed at user scope from the DevFlow marketplace. Project skills
must not edit `~/.codex`, `~/.agents`, or another user-level configuration. The
project's portable provider preference lives only in `.planning/config.json`.
