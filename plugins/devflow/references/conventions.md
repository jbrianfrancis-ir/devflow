...
**Forbidden-pattern regexes.** The same pre-commit and pre-push scans additionally apply any regex declared on `.planning/ARCHITECTURE.md`'s Forbidden entries (syntax: `templates/architecture.md`), run across **tracked files, including `.planning/`** — the secret scan above is scoped to the diff, but a forbidden literal that reached `.planning/` in an earlier commit is exactly the case this exists to catch. Same reporting rule as above, in force: file + line + pattern class only — **never echo the matched value** — fail closed, only a human clears a hit. A Forbidden entry with no regex is **not enforced** by this or any other check — it stays prose guidance, and reading it as machine-checked when it isn't is the defect this exists to close.
