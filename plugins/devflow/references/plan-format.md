...
**Split signals** — always split into more plans when: more than 4 tasks; multiple subsystems (DB + API + UI = separate plans); any task touching >5 files; discovery mixed with implementation; checkpoint mixed with implementation. `<falsify>` clauses add bytes and count against the 4KB cap — split rather than trim a falsify away. Never shrink scope to fit a plan — split instead, with `plugins/devflow/scripts/flow-split-plan.py`.
