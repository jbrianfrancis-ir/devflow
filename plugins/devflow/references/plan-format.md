...
A plan is an executor prompt: complete, unambiguous, executable by an agent with no other context. File: `.planning/phases/NN-slug/NN-MM-PLAN.md`, structure in `templates/plan.md`. **4KB is a soft target, not a gate.** Measure it once with `wc -c` when the plans are on disk — a byte count is not a judgment call, and a planner that re-measures its own prose after every edit burns a top tier on arithmetic. A plan over the target is a signal to **split** (`{devflow_root}/scripts/flow-split-plan.py`), never a reason to spend a revision round: never re-run the planner, and never trim prose, to chase a byte count. Trimming to fit degrades the plan the target exists to protect — repeated hand-trimming under byte pressure is what produced two contradicting truths in the phase this rule came from (one asserting credentials repopulate on relaunch, another forbidding it, because the trimming pass that made each plan fit never checked what the other plan still said). If splitting is not natural, the plan ships over target and the size is reported once, not re-litigated. The split signals below are what actually keep a plan near it.

## Frontmatter
All required unless noted: `phase`, `plan`, `wave`, `depends_on`, `files_modified`, `autonomous`, `requirements` (REQ-IDs from the roadmap — never empty), `must_haves.{truths,artifacts,key_links}` plus optional `must_haves.backstop_truths`. Optional `user_setup`: external things the human must configure (accounts, secrets) — surfaced before execution starts. Optional `files_new`: every file this plan **creates** — a subset of `files_modified`, and required for any file the plan creates.

**Plan lint.** `{devflow_root}/scripts/flow-plan-lint.py <phase-dir>` checks the rules above deterministically: required keys (R1), `depends_on` resolves (R2), wave arithmetic (R3), no cycles (R4), same-wave `files_modified` disjoint (R5), every `files_modified` path exists at `HEAD` or is in `files_new` of this plan or a plan it transitively depends on (R6), and every repo path in `<context>` exists the same way (R7). It fails closed: a plan it cannot read, or a repo without a resolvable `HEAD`, is an `R0 could-not-check` finding, never a pass. `/flow-plan` runs it in the revision gate and `/flow-execute` in pre-flight.
...
**Split signals** — always split into more plans when: more than 4 tasks; multiple subsystems (DB + API + UI = separate plans); any task touching >5 files; discovery mixed with implementation; checkpoint mixed with implementation. `<falsify>` clauses add real bytes — split rather than trim a falsify away, and never drop one to fit the soft target. Never shrink scope to fit a plan — split instead: `python3 {devflow_root}/scripts/flow-split-plan.py <phase-dir> <plan> --after K`.
