...
## Smoke gate — the whole app, not this phase
Every check above is scoped to *this* phase's truths, which is exactly why a phase can pass while having broken an earlier one. The smoke gate is the standing counterweight: it asks whether the application still works end to end, and it runs on every phase regardless of what the phase touched.

Read the `## Smoke` section of `.planning/ARCHITECTURE.md` and run its command verbatim. Judge it against the declared "Pass looks like" — not against your own sense of whether the output seems fine.
- **Pass** → record the command and its result as the gate's evidence and continue.
- **Fail** → a `GAP`, phrased as the regression it is (`smoke: <what broke>`), even when every phase truth VERIFIED. Status is `gaps`. This is the one gap that is routinely **not** caused by the phase under verification; say so when the evidence points at earlier work, so replanning targets the right phase.
- **Not declared** (`none (not yet defined)`, missing section, or no ARCHITECTURE.md) → never invent a command and never quietly skip. Add a `HUMAN` check: *declare a smoke command in ARCHITECTURE.md — no end-to-end regression gate exists*. One standing item, not a per-phase nag.

A smoke failure is infrastructure-shaped as often as code-shaped (a service that won't start, a missing migration, a stale container). Report what the command did, not a theory about why — diagnosis is `/flow-debug`'s job, not the verifier's.
