...
4. **Write** from templates at `{devflow_root}/templates/` (architecture.md, project.md, requirements.md, roadmap.md, state.md):
   ...
   - `config.json` — `{"mode":"interactive","commit_docs":true,"agents":{"provider":"native"},"deploy":{"tool":"aspire+azd"},"git":{"base":"<dev|main>","origin":"origin","upstream":"<upstream|null>","branch":"flow/<slug>"}}` (`--auto` → `"mode":"auto"`; an explicit `--provider` sets `agents.provider` to that value). **Set `deploy.tool` to `null` when the project has no deployable surface** — a library, CLI, plugin, or docs repo ships by merging, not by deploying. It turns off the harden/UAT/release chain for every later routing decision (autonomy.md → Projects with no deployable surface); record the matching `D-NN` in PROJECT.md. Leave the default when the project deploys, or when you are unsure — the ambiguous case takes the audit.
   - `JOURNAL.md` — from `{devflow_root}/templates/journal.md`, one init line.
   - `DECISIONS.md` — from `{devflow_root}/templates/decisions.md`, header only (no entries until the first human gate). Append-only and uncapped; see conventions.md (Decision log).
...
5. **Commit** (if commit_docs): `chore(flow): initialize project` on the feature branch, then `git push -u origin <branch>` (the push canary — an auth failure is a GATE per conventions.md Credential modes, not a retry loop). Print the roadmap table, then the autonomy recipes:
   - Drive to completion: `/goal FLOW says DONE, GATE, or BLOCKED, or stop after 40 turns` then `/flow-next`
