...
Two things this must not claim. The newest version *this machine knows of* is not the published release — the marketplace cache lags until Claude Code refreshes it, so matching it is not proof of being current, and the line should say "newest known" rather than "latest". And when the registry is missing entirely there is no Claude plugin system here (Codex, a container) — that is not applicable, not a failure; when it exists but will not parse, say the check did not run (`conventions.md` → Fail-closed guards). Never report a version you did not read.
