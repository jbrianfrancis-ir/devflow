...
2a. **Telemetry gate** (pre-deploy): invoke `telemetry-readiness --scope all --env uat` with the host skill mechanism. Overall `FAIL` stops the deploy — `FLOW: GATE`, with the blocking ids; the fix route is `/flow-quick` or `/flow-plan N --gaps`, and a human override is a `.planning/DECISIONS.md` entry, never yours to assume. Overall `UNVERIFIED` is also a GATE: ask for platform access or an explicit deploy-with-UNVERIFIED-telemetry decision. On a first-ever deploy to this environment P4 and P5 are UNVERIFIED by construction — there is no revision yet — so say so and re-run them in step 4 rather than treating them as the gap.

3. **Deploy** per the reference: `azd env list` first, and **that answer decides alone** — the PIPELINE uat row is a cache of Azure's fact, not a second vote (`autonomy.md` → External state is a cache, never evidence). Env **absent** → `azd init` (if needed), `azd env new uat`, `azd env select uat`, `azd up` (azd prompts the user for parameters/secrets — let it). Env **present** → `azd env select uat`, then `azd provision` only if the AppHost/infra model changed since the last uat deploy (check git diff on AppHost project), then `azd deploy`. `azd env list` failed → say the live check did not run and take the **present** branch; `azd env select` failing is a cheap, recoverable way to discover the env is absent, whereas guessing "absent" runs `azd up` over an environment that already exists. Complete that fallthrough rather than stalling on it: `azd env select uat` failing **not-found** means the env really was absent → take the absent branch (`azd init` if needed, `azd env new uat`, `azd up`). Any other `azd env select` failure is an auth or infra error, not an absent env — apply the reference's failure table and never `azd env new` on it.

4. **Post-deploy**: capture endpoint URLs from azd output into UAT-PLAN frontmatter and PIPELINE.md; curl the health endpoints (smoke). **Readiness, not sleeps**: poll the health endpoint until healthy or a deadline (~3 min) — never fixed sleeps; report the failure the moment it's definitive. Web UI → run the **route sweep**: load each Route-sweep row against the live UAT URL (headless browser when available — capture console errors and failed network requests; else curl status + body sanity) and fill the table — a console error or failed request fails the row even if the page renders. Then re-run `telemetry-readiness --scope platform --env uat` — P2–P5 against the revision that just went out: the connection string is on it, and telemetry is arriving. This is the only moment P4 and P5 can pass on a first deploy. A `FAIL` here does not un-deploy uat; it is a GATE before `/flow-release`, with its own DECISIONS.md entry if it is overridden. Update the PIPELINE uat row (SHA, date, result). Deploy or smoke failure → apply the reference's failure table; fixes go through `/flow-quick`; then redeploy.
