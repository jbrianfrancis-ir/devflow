...
3. **Deploy** per the reference: `azd env list` first, and **that answer decides alone** — the PIPELINE uat row is a cache of Azure's fact, not a second vote (`autonomy.md` → External state is a cache, never evidence). Env **absent** → `azd init` (if needed), `azd env new uat`, `azd env select uat`, `azd up` (azd prompts the user for parameters/secrets — let it). Env **present** → `azd env select uat`, then `azd provision` only if the AppHost/infra model changed since the last uat deploy (check git diff on AppHost project), then `azd deploy`. `azd env list` failed → say the live check did not run and take the **present** branch; `azd env select` failing is a cheap, recoverable way to discover the env is absent, whereas guessing "absent" runs `azd up` over an environment that already exists.
