...
2. **Preview**: inventory `.planning/` top-level + phase dirs (names and counts only — don't read bodies here). Resolve the **GSD skill roster** this host has installed (your own skill listing is authoritative; else glob the host skill directories for `gsd-*`) — step 3 writes it to `.claude/settings.json` as `skillOverrides`. Show the user the plan: what converts, what archives, the resumed position, how many `/gsd-*` skills get disabled in this repo, and this warning: **after migration, never run `/gsd-*` commands in this project again** (both systems own `.planning/`) — enforced by the `skillOverrides` block in Claude Code, advisory only in Codex, which has no equivalent setting. Get explicit confirmation — this is a GATE even in auto mode.

3. **Migrate**: spawn `flow-migrator` with paths: `{devflow_root}/references/migrate-gsd.md`, `{devflow_root}/templates/` (dir), `{devflow_root}/references/conventions.md`, and the repo root — plus the step-2 GSD skill roster (an empty roster means write no `skillOverrides` key, not an empty one). Keep only its ≤15-line report in context.
...
5. **Verify the migration**: check `.claude/settings.json` still parses as JSON and carries the bootstrap keys, a `skillOverrides: "off"` entry for every name in the step-2 roster, and every key it held before the migration (merged, never overwritten — a settings file the migration corrupted breaks the project for every future session, so this is checked before the planning files); check every new `.planning/` file exists and is within its template cap; STATE.md position matches the migrator's report; requirement IDs in ROADMAP all exist in REQUIREMENTS.md; archive dir contains everything that isn't converted (nothing vanished: file count in ≈ converted + archived). Any mismatch → BLOCKED, fix before committing.
