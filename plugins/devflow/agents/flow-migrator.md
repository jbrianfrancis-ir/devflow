...
After writing, re-read the file: confirm it parses, and diff its key set against the pre-migration snapshot whose path your prompt carries. Report any key present in the snapshot and absent now — do not repair it silently. If you were given no snapshot path and the file already existed, say so plainly and treat the merge as unverified; a settings file you corrupted breaks every future session in this repo, and it is the one file here you cannot rebuild from the archive.
