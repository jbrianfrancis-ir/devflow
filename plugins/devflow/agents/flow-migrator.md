...
5b. **Self-bootstrap**: merge the plugin self-bootstrap block (conventions.md, Plugin self-bootstrap section) into `.claude/settings.json` so any future session — including cloud — installs DevFlow automatically. Merge keys only; preserve everything else in the file (except the GSD entries removed in step 5).
