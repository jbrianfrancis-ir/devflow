...
1b. **Unresolved markers** (when REQUIREMENTS.md is in your inputs) — for each `[NEEDS CLARIFICATION: …]` still open on this phase's requirements, the plan that implements it carries a matching entry in `must_haves.backstop_truths`. A plan that silently resolved a marker — picking an answer with no backstop truth, or editing the marker away — is an issue. **Resolved markers**: for each marker RESOLVED (plan-format.md's backstop_truths resolution rule), the resolution appears as a `must_haves.truths` entry in the plan implementing it, and no plan in the phase states something that resolution contradicts — report **every** contradicting site, not the first; a partial list reads as a complete one. A resolution recorded only as a YAML comment, an objective note, or a `<human-check>` is an issue.
...
5b. **Verify can fail** (plan-format.md: negative control, execution-only assertion) — a command `<verify>` with no `<falsify>`, or a `<falsify>` naming no concrete mutation ("make it fail", "break the input", a restatement of the verify), is blocking; `<human-check>`-only verifies are exempt. A `<verify>` whose pass condition would also hold in the state it exists to catch is blocking too — watch for a `grep` passing on empty output (also what a missing file, a wrong path, or exit 2 produces) and, when `files_modified` adds a test, a verify resting on an aggregate marker or a bare suite name rather than a string only the runner emits. Quote the mutation or log line that defeats the check.
6. **Size** — >4 tasks, or discovery mixed with implementation, or checkpoint mixed with implementation → must split.
6b. **Cross-plan references** — resolve every `NN-MM` mention in plan prose. A reference to a plan that does not exist in this phase is blocking. A reference asserting another plan does X, where X appears in neither that plan's `files_modified` nor its `must_haves.truths`, is blocking — an executor following it leaves work parked on a plan that will never do it.
