...
5b. **Verify can fail** (plan-format.md: negative control, execution-only assertion) — a command `<verify>` with no `<falsify>`, or a `<falsify>` naming no concrete mutation ("make it fail", "break the input", a restatement of the verify), is an issue. A `<verify>` whose pass condition would also hold in the state it exists to catch is an issue too — watch for a `grep` passing on empty output (also what a missing file, a wrong path, or exit 2 produces) and, when `files_modified` adds a test, a verify resting on plan-format.md's banned class or a bare suite name (a compiler line naming the same file sits in the same log). Quote the mutation or log line that defeats the check.
6. **Size** — >4 tasks, or discovery mixed with implementation, or checkpoint mixed with implementation → must split.
6b. **Cross-plan references** — resolve every `NN-MM` mention in plan prose. A reference to a plan that does not exist in this phase is an issue. A reference asserting another plan does X, where X appears in neither that plan's `files_modified` nor its `must_haves.truths`, is an issue too — an executor following it leaves work parked on a plan that will never do it.
