...
4. **Verify** after the last wave: spawn `flow-verifier` with the phase dir, `${CLAUDE_PLUGIN_ROOT}/references/verification.md`, and `${CLAUDE_PLUGIN_ROOT}/templates/verification.md`. The verifier gets a fresh context and only artifacts on disk — never executor conversations or your run notes; a checker that shares the worker's context is grading its own homework.
