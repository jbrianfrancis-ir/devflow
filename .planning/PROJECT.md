...
| D-21 | Skills may declare `hooks` in SKILL.md frontmatter — skill-scoped, `type: prompt` only, degrading on hosts without them, structurally validated | Reverses the standing "DevFlow ships no hooks" property. `/flow-handsoff` needs the session `Stop` hook `/goal` registers, and a built-in CLI command cannot be invoked by a skill. Constrained rather than opened: no executable hook scripts, so the no-Node-runtime property is untouched | 2026-08-21 |
