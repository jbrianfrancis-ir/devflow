...
human_checks: []
deferred:
  - "5 pre-existing SecretScanGuardTests failures (183 tests), on main before this plan and
    unchanged after — NOT unrelated: root-caused after execution to `diff.mnemonicPrefix=true` in git config, which makes git emit `diff --git c/f w/f`
    while secret-scan-guard.py's DIFF_GIT_HEADER_RE hardcodes `a/`/`b/`. Zero file chunks
    parsed, exit 0 (allow), silently. Live fail-open; own quick, own bump."
---
